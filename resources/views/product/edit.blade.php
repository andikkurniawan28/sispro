...
@section('title')
    {{ ucwords(str_replace('_', ' ', 'edit_product')) }}
@endsection
                    ...
                    <div class="card-body">
                        <form action="{{ route("product.update", $product->id) }}" method="POST">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" id="name" name="name" value="{{ old("name", $product->name) }}" required autofocus>
                                </div>
                            </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <input type="number" class="form-control" id="expiration_time" name="expiration_time" value="{{ old("expiration_time", $product->expiration_time) }}" required>
                                </div>
                            </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <select class="product_category_id" id="product_category_id" name="product_category_id" required>
                                        <option disabled>Select a {{ str_replace("_", " ", "product_category") }} :</option>
                                        @foreach ($product_categories as $product_category)
                                            <option value="{{ $product_category->id }}" {{ $product->product_category_id == $product_category->id ? 'selected' : '' }}>
                                                @php echo ucwords(str_replace('_', ' ', $product_category->name)); @endphp
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <select class="unit_id" id="unit_id" name="unit_id" required>
                                        <option disabled>Select a unit :</option>
                                        @foreach ($units as $unit)
                                            <option value="{{ $unit->id }}" {{ $product->unit_id == $unit->id ? 'selected' : '' }}>
                                                {{ $unit->name }} (@php echo str_replace('_', ' ', $unit->symbol); @endphp)
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <select class="product_status_id" id="product_status_id" name="product_status_id" required>
                                        <option disabled>Select a {{ str_replace("_", " ", "product_status") }} :</option>
                                        @foreach ($product_statuses as $product_status)
                                            <option value="{{ $product_status->id }}" {{ $product->product_status_id == $product_status->id ? 'selected' : '' }}>
                                                @php echo ucwords(str_replace('_', ' ', $product_status->name)); @endphp
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
