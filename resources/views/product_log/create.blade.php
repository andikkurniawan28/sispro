...
@section('title')
    {{ ucwords(str_replace('_', ' ', 'create_product_log')) }}
@endsection
        ...
        <div class="row">
            <div class="col-xxl">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <form action="{{ route('product_log.store') }}" method="POST">
                            @csrf
                            @method('POST')
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-10">
                                    <select class="product_warehouse_id" id="product_warehouse_id" name="product_warehouse_id" required autofocus>
                                        <option disabled selected>Select a {{ ucwords(str_replace("_", " ", "product_warehouse")) }} :</option>
                                        @foreach ($product_warehouses as $product_warehouse)
                                            <option value="{{ $product_warehouse->id }}">@php echo ucwords(str_replace('_', ' ', $product_warehouse->name)); @endphp</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                        ...
                                        <div class="row mb-3" id="product_row_{{ $index }}">
                                            <div class="col-sm-4">
                                                ...
                                                <select class="product form-control" id="product_{{ $index }}" name="products[]" required>
                                                    ...
                                                    @foreach ($products as $productItem)
                                                        <option value="{{ $productItem->id }}" {{ $productItem->id == $product ? 'selected' : '' }}>
                                            ...
                                            <div class="col-sm-2">
                                                ...
                                                <span class="unit_symbol form-control" id="unit_symbol_{{ $index }}">
                                                    {{ $products->find($product)->unit->symbol }}
                                                </span>
                                            </div>
                                            <div class="col-sm-2">
...
@section('additional_script')
    ...
    <script>
        $(document).ready(function() {
            ...
            let productCounter = {{ old('products') ? count(old('products')) : 0 }};
