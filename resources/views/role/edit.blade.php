                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <form action="{{ route("role.update", $role->id) }}" method="POST">
                            ...
                            <div class="row mb-3">
                                <label class="col-sm-2 col-form-label" for="department_id">
                                    {{ ucwords(str_replace('_', ' ', 'department')) }}
                                </label>
                                <div class="col-sm-10">
                                    <select class="department_id" id="department_id" name="department_id" required autofocus>
                                        <option disabled selected>Select a {{ ucwords(str_replace("_", " ", "department")) }} :</option>
                                        @foreach ($departments as $department)
                                            <option value="{{ $department->id }}"
                                                {{ $department->id == $role->department_id ? 'selected' : '' }}>
                                                @php echo ucwords(str_replace('_', ' ', $department->name)); @endphp
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
