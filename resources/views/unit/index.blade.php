        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="btn-group" role="group" aria-label="manage">
                    <a href="{{ route('unit.create') }}" class="btn btn-sm btn-primary">Create</a>
                </div>
                    ...
                    <table class="table table-hover table-bordered" id="example" width="100%">
                        ...
                        <tbody>
                            @foreach ($units as $unit)
                                <tr>
                                    <td>{{ $unit->name }}</td>
                                    <td>@php echo $unit->symbol; @endphp</td>
                                    <td>
                                        <div class="btn-group" role="group" aria-label="manage">
                                            ...
                                            <button type="button" class="btn btn-danger btn-sm delete-btn"
                                                data-id="{{ $unit->id }}"
                                                data-name="{{ $unit->name }}">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.addEventListener('click', function(event) {
                if (event.target.classList.contains('delete-btn')) {
                    ...
                    const unit_id = button.getAttribute('data-id');
                    const unit_name = button.getAttribute('data-name');
                    Swal.fire({
                        title: 'Are you sure?',
                        ...
                        if (result.isConfirmed) {
                            ...
                            name.setAttribute('value', unit_name);
